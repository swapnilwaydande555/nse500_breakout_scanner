...
# Alpha Vantage key (optional) - set ALPHA_VANTAGE_KEY as env var or Streamlit secret
ALPHA_KEY = os.getenv("ALPHA_VANTAGE_KEY", "").strip()

# Headers for NSE fallback
HEADERS = {
...
def analyze_symbol(symbol):
    ...
    try:
        ...
        if weekly is not None and not weekly.empty:
            ...
            weekly = compute_indicators(daily.resample("W").agg({
        ...
        if daily_break:
            reasons.append("Daily breakout"); confidence += 0.2
        if weekly_break:
            reasons.append("Weekly breakout"); confidence += 0.35
        if rsi_ok:
            reasons.append("RSI>55"); confidence += 0.15
        if macd_ok:
            reasons.append("MACD positive"); confidence += 0.1
        if ema_trend:
            reasons.append("EMA trend up"); confidence += 0.1
        if vwap_ok:
            reasons.append("Above VWAP"); confidence += 0.05
        if vol_ok:
            reasons.append("Volume surge"); confidence += 0.15
        else:
            reasons.append("Volume low - watch fakeout"); confidence -= 0.1
        ...
        if weekly_break and confidence > 0.7:
            holding = "Long (1-6 months)"; holding_reason = "Weekly breakout with strong momentum"
        elif confidence > 0.8 and ema_trend and macd_ok:
            holding = "Medium (1-4 weeks)"; holding_reason = "High confidence and trend alignment"
        else:
            holding = "Short (2-7 days)"; holding_reason = "Moderate confidence - prefer quick review"
